...
- **Root Cause:** Strict backend validation in `OrderService` and `PaymentService` that required an `OPEN` cash closure for _any_ order/payment creation, without considering the transaction date.
