import { Payment, PaymentMethod, OrderStatus, TicketBook } from "@prisma/client";
...
export class PaymentService {
  ...
  async createPayment(orderId: string, data: { method: PaymentMethod; amount: number; transactionRef?: string; phone?: string }): Promise<Payment> {
    return await this.prisma.$transaction(async (tx: PrismaTransaction) => {
      ...
      if (!order) throw new CustomError("Order not found", HttpStatus.NOT_FOUND);
      ...
      if (data.method === PaymentMethod.TICKET_BOOK) {
        if (!data.phone) throw new CustomError("Phone required for ticket book payment", HttpStatus.BAD_REQUEST);
        
        const customer = await this.customerRepo.findByPhoneWithActiveTickets(data.phone);
        if (!customer) throw new CustomError("Customer not found", HttpStatus.NOT_FOUND);

        const activeTicket = customer.ticketBooks.find((tb: any) => tb.consumedPortions < tb.totalPortions);
        if (!activeTicket) throw new CustomError("No active ticket book with portions available", HttpStatus.BAD_REQUEST);
