...
export class CustomerController {
  ...
  getCustomerByPhoneWithTickets = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { phone } = phoneSearchSchema.parse(req.params);
      const customer = await this.customerService.findByPhoneWithActiveTickets(phone);
      res.status(200).json({ success: true, message: "Customer retrieved successfully", data: customer });
    } catch (error) { next(error); }
  };
  ...
  searchCustomers = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: "Customers searched successfully", data: result.data, pagination: result.pagination });
    } catch (error) { next(error); }
  };
  ...
  listCustomers = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      res.status(200).json({ success: true, message: "Customers listed successfully", data: result.data, pagination: result.pagination });
    } catch (error) { next(error); }
  };
