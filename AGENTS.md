...
- _Note: Client currently lacks a standard test suite; focus on `type-check` and `lint` for the frontend._
...
- **Validation:** Use **Zod** for all data validation across both environments.
  - _Client:_ Always use `react-hook-form` combined with `@hookform/resolvers/zod`.
  - _Server:_ Every request (body, query, params) must be validated at the route level via Zod middleware.
